...
import itertools
import torch
torch.cuda.set_device(1)  # 使用物理GPU1（需在加载模型前调用）

# 在import区域添加
from PIL import ImageFile
ImageFile.LOAD_TRUNCATED_IMAGES = True #允许加载被截断的图片

from register_dataset import register_datasets  #引入自定义数据集函数
from typing import Any, Dict, List, Set
import torch

#显存
torch.cuda.empty_cache()  #释放pytorch空闲的预留显存
# 启用动态显存分配
...
import numpy as np
#numpy与detectron2版本问题, numpy > 1.20之后不再使用np.bool
if not hasattr(np, 'bool'):
    np.bool = bool

from detectron2.data import MetadataCatalog, DatasetCatalog
from detectron2.data.datasets import register_coco_instances  ##用于自定义数据集
        ...
        if evaluator_type in ["sem_seg", "coco_panoptic_seg"]:
            evaluator_list.append(
                SemSegEvaluator(
                    dataset_name,
                    distributed=True,
                    num_classes= 2, #cfg.MODEL.SEM_SEG_HEAD.NUM_CLASSES,  实例分割类别数
...
if __name__ == "__main__":   #分布式训练参数（gpu）
    args = default_argument_parser().parse_args()
    print("Command Line Args:", args)
    launch(
        main,
        1 ,#args.num_gpus,  # 这里自动指定 GPU 数量
        num_machines=1,
        machine_rank=0,
        dist_url= "tcp://127.0.0.1:12355",#args.dist_url,
        args=(args,),
    )
