...
register_coco_instances(
    "my_dataset_test",  # 数据集名称
    {},  # 元数据（可选）
    "xxxx/xxx/test/annotations.json",  # 标注文件路径
    "xxx/xxxx/test/images"  # 图像文件夹路径
)
...
def test_sparseinst_speed(cfg, fp16=False):
    ...
    with autocast(enabled=fp16):
        with torch.no_grad():
            for idx, inputs in enumerate(data_loader):
                ...
                if idx > 0:  # 跳过第一个 batch，避免初始化时间影响
                    latency = np.mean(durations[1:])  # 跳过第一个 batch
                    fps = 1 / latency
    ...
    if len(durations) > 1:  # 确保有足够的数据计算 FPS
        latency = np.mean(durations[1:])  # 跳过第一个 batch
        fps = 1 / latency
        print("speed: {:.4f}s FPS: {:.2f}".format(latency, fps))
    else:
        print("Warning: Not enough data to calculate FPS.")
...
def setup(args):
    ...
    cfg.DATASETS.TEST = ("my_dataset_val",)  # 设置验证集名称
    cfg.freeze()
