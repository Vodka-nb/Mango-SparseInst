...
class CoordAtt(nn.Module):
    ...
    def forward(self, x):
        ...
        # 特征融合
        feat = self.conv1(torch.cat([x_h, x_w], dim=2))
        feat = self.bn1(feat)
        feat = self.act(feat)
        # 分解特征
        feat_h, feat_w = torch.split(feat, [h, w], dim=2)
        feat_w = feat_w.permute(0, 1, 3, 2)
        # 生成注意力权重
        att_h = torch.sigmoid(self.conv_h(feat_h))
        att_w = torch.sigmoid(self.conv_w(feat_w))
...
class DeformableBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, cardinality=1, base_width=64,
        ...
        outplanes = planes * self.expansion  #定义 outplanes
        if attn_layer is not None:
            self.attn = attn_layer(outplanes)
        else:
            self.attn = None
...
def make_blocks(
        ...
        down_kernel_size=1, avg_down=False, drop_block_rate=0., drop_path_rate=0.,attn_layer=CoordAtt,  **kwargs):
    stages = []
    feature_info = []
    ...
    attn_layer = [None, None, CoordAtt, CoordAtt]

    for stage_idx, (planes, num_blocks, db) in enumerate(zip(channels, block_repeats, drop_blocks(drop_block_rate))):
        # choose block_fn through the BLOCK_TYPE
        block_fn = BLOCK_TYPE[stage_block[stage_idx]]

        attn = attn_layer[stage_idx] if isinstance(attn_layer, (list, tuple)) else attn_layer
        ...
        block_kwargs = dict(reduce_first=reduce_first, dilation=dilation, drop_block=db,attn_layer=attn ,**kwargs)
        blocks = []
