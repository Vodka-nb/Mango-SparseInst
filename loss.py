...
# 自定义损失函数
def custom_loss(predictions, targets, small_object_class):
    # 计算交叉熵损失
    loss_ce = F.cross_entropy(predictions, targets)

    # 计算细小物体的权重
    small_object_weight = (targets == small_object_class).float().mean()

    # 加权损失
    total_loss = loss_ce * (1 + small_object_weight)
    return total_loss
...
class SparseInstCriterion(nn.Module):
    ...
    def get_loss(self, loss, outputs, targets, indices, num_instances, **kwargs):
        loss_map = {
            ...
            "custom_loss": self.custom_loss,  # 添加自定义损失
        }
    ...
    def custom_loss(self, outputs, targets, indices, num_instances, input_shape=None):
        # 这里假设您有一个特定的类来计算小物体的损失
        small_object_class = 1  # 例如，假设类1是树枝
        predictions = outputs['pred_logits']
        targets_labels = torch.cat([t["labels"] for t in targets])
        return custom_loss(predictions, targets_labels, small_object_class)
